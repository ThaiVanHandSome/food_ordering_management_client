...
export default function OrderProductSheet() {
  const { productOrders, setProductOrders, customerName, tableNumber, customerId } = useContext(AppContext)
          ...
          <ScrollArea className='flex-1'>
            <div className='h-full px-2'>
              {productOrders?.map((productOrder) => (
                <div key={productOrder.product._id} className='mb-4 flex items-center justify-between'>
                  <div className='flex items-center'>
                    ...
                    <div className='ml-2'>
                      ...
                      <p className='text-red-700 font-bold'>{formatCurrency(productOrder.product.price)}đ</p>
                    </div>
                  </div>
                  <div>
                    <QuantityController value={productOrder.buy_count} setValue={setValue(productOrder.product._id)} />
                  </div>
                </div>
              ))}
            </div>
