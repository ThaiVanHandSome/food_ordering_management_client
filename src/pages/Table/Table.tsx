...
import { setCustomerIdToLocalStorage, setCustomerNameToLocalStorage, setTableNumberToLocalStorage } from '@/utils/auth'
...
export default function Table() {
  ...
  const { setTableNumber, setCustomerName, setCustomerId } = useContext(AppContext)
  ...
  return (
    <div className='flex justify-center'>
      <div className='w-1/4 border rounded-lg px-6 py-4 mt-6'>
        <h1 className='font-bold text-center mb-2 text-xl'>Đăng nhập gọi món</h1>
        <Form {...form}>
          <form onSubmit={onSubmit}>
            <InputCustom control={form.control} name='name' placeholder='Tên' label='Tên' />
