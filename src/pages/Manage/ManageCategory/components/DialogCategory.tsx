...
export default function DialogCategory({ category }: Props) {
  ...
  const updateCategoryMutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: { name: string } }) => updateCategory(id, body)
  })
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isUpdate ? 'Chỉnh sửa danh mục' : 'Thêm danh mục'}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={onSubmit}>
            <InputCustom control={form.control} name='name' placeholder='Tên danh mục' label='Tên danh mục' />
