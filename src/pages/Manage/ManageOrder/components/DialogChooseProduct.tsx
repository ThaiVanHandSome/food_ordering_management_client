...
export default function DialogChooseProduct({ setProduct, isUpdate = true }: Props) {
  const form = useForm({
    defaultValues: {
      name: ''
    }
          ...
          <Table>
            ...
            <TableBody>
              {products?.data.data.content.map((product) => (
                <TableRow className='cursor-pointer' onClick={() => handleSetNewProduct(product)}>
                  <TableCell>
                    <img src={product.image} alt={product.name} className='size-16 object-cover rounded-lg' />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <p className='text-destructive font-bold'>{formatCurrency(product.price)}đ</p>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
